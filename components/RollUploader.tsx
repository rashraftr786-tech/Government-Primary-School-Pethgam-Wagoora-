...
const aliases: Record<string, keyof StudentRow> = {
  ...
  aadhaar_apaar_id: "aadhaarApaarId",
  aadhaarapaarid: "aadhaarApaarId",
  ...
  cwsn_status: "cwsnStatus",
  cwsnstatus: "cwsnStatus",
};

function normalizeBoolean(value: unknown) {
  return ["yes", "y", "true", "1", "present"].includes(
    String(value ?? "")
      .trim()
      .toLowerCase(),
  );
  ...
  rawRows.forEach((raw, index) => {
    ...
    if (
      ...
      errors.push({
        row: rowNumber,
        message:
          "Roll No, Student Name and Class (1–5) are required.",
      });
    ...
    if (seen.has(key)) {
      errors.push({
        row: rowNumber,
        message: `Duplicate Roll No ${rollNo} in Class ${classNo}.`,
      });
    ...
    valid.push({
      ...
      category:
        String(normalized.category ?? "").trim() || undefined,
      bplStatus: normalizeBoolean(normalized.bplStatus),
      aadhaarApaarId:
        String(normalized.aadhaarApaarId ?? "").trim() || undefined,
      parentName:
        String(normalized.parentName ?? "").trim() || undefined,
      contactNumber:
        String(normalized.contactNumber ?? "").trim() || undefined,
...
export default function RollUploader() {
  ...
  async function handleFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      /*
       * XLSX can read both Excel files and CSV files.
       * Therefore PapaParse is no longer required.
       */
      ...
      if (!raw.length) {
        throw new Error(
          "The uploaded file is empty or contains no student records.",
        );
      }
      ...
      setStatus(
        `Validated ${result.valid.length} students. ${result.errors.length} row(s) need correction.`,
      );
    } catch (error) {
      setRows([]);
  ...
  async function saveRows() {
    ...
    try {
      ...
      setStatus(
        `Saved ${rows.length} students successfully.`,
      );
    } catch {
      await queueMutation({
      ...
      setStatus(
        "Offline: students queued locally and will sync when connection returns.",
      );
    }
  }
    ...
    <div className="space-y-5">
      <div>
        ...
        <h2 className="text-2xl font-bold">
          Dynamic Student Roll
        </h2>

        <p className="mt-1 text-sm text-slate-500">
          Upload XLSX or CSV, validate rows, then commit the
          clean dataset.
        </p>
      </div>
      ...
      <label className="flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-300 bg-white p-8 text-center hover:border-school-600">
        <UploadCloud
          className="mb-3 text-school-600"
          size={32}
        />

        <span className="font-semibold">
          Choose Student Roll .xlsx or .csv
        </span>

        <span className="mt-1 text-xs text-slate-500">
          Columns: Roll_No, Student_Name, Class, Gender,
          Category, BPL_Status, Aadhaar_APAAR_ID, Parent_Name,
          Contact_Number, CWSN_Status
        </span>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <button
          ...
          className="inline-flex items-center gap-2 rounded-lg bg-school-600 px-4 py-2.5 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50"
        >
